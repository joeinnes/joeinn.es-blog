<script lang="ts">
  ...
  $: {
    ...
    seatsPerVoteShare = Math.round(
      (voteStats.votes / totals[year].votesForWinningParties) * 650
    );
...
{#if votedFor}
  ...
  <p>
    ...
    {:else}
      If the votes of everyone in your party had been as strong as an average
      vote, then your party would have received {seatsPerVoteShare} seats, instead
      of {voteStats?.seats}.
    {/if}
  </p>
{/if}
